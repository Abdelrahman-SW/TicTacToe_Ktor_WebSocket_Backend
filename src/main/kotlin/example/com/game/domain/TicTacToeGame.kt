...
class TicTacToeGame {
    ...
    private val countDownInterval = 5
    ...
    fun connectPlayer(player: Player, session: WebSocketSession): Boolean {
        ...
        state.update {
            ...
            val playerAtTurn = getRandomPlayer(connectedPlayers)
    ...
    fun resetGame() {
        state.update {
            it.copy(
                playerAtTurn = getRandomPlayer(it.connectedPlayers),
    ...
    private fun startNewGameDelayed() {
        delayJob?.cancel()
        delayJob = gameScope.launch {
            state.update {
                it.copy(countdown = countDownInterval)
            }
            for (i in countDownInterval - 1 downTo  0) {
                delay(1000)
                state.update {
                    it.copy(countdown = i)
                }
...
private fun getRandomPlayer(connectedPlayers: List<Player>): Player? {
    return if (connectedPlayers.size == 2) connectedPlayers.random() else null
}
...
fun makeTurn(player: Player, turn: PlayerTurn) {
    ...
    state.update { currentGameState ->
        ...
        if (isBoardFull || winingPlayer != null) {
            startNewGameDelayed()
        }
